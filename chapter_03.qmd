...
(From @callout-angmom_scalar). Then,
...
If the object in question passes perigee at $t_p=0$, and use @eq-rpos then the equation becomes:
...
Here we can find from integral tables that:

$$
\int_0^\theta \frac{d\theta}{(1+e\cos{\theta})^2}= \frac{1}{(1-e^2)^{\frac{3}{2}}}\left[2\tan^{-1}{\left(\sqrt{\frac{1-e}{1+e}}\tan{\frac{\theta}{2}}\right)}-\frac{e\sqrt{1-e^2}}{1+e\cos{\theta}}\sin{\theta}\right]
$$